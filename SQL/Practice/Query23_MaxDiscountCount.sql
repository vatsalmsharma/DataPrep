...
-- Finding max number of discount a family and obtain
--script:
...
-- Based on Country table's min size
with cte as (
            select f.name as family_name, c.name as country_name, c.min_size, f.family_size 
            from families f , countries c 
            where c.min_size <= f.family_size)
, scope as (
            select family_name, count(*) as my_count
            from cte
            group by family_name
           )
select max(my_count) from scope;


-- Based on Country table's min size and max size
with cte as (
            select f.name as family_name, c.name as country_name, c.min_size, c.max_size, f.family_size 
            from families f , countries c 
